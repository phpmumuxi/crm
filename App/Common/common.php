...
function getIndexFields($model){
    ...
    $where['in_index'] = 1;
    $where['model'] = $model;
    $model_fields = M('Fields')->where($where)->order('order_id ASC')->cache(true)->select();
    //dump($model_fields);
    return $model_fields;
}
//获取主表字段 用于搜索
function getMainFields($model){
    ...
    unset($model_fields['1']);
    unset($model_fields['9']);
    return $model_fields;
}
/**记录操作日志
 * $id 操作对象id
...
function actionLog($id,$param_name='',$text=''){
    ...
    //dump($user);
    $category = $user['category_id'] == 1 ? L('ADMIN') :'';
    // dump($category);
    ...
    $data['action_id'] = $id;
    // dump(array($category,$user['name'],date('Y-m-d H:i:s'),L($data['action_name']),$id,L(MODULE_NAME),$text));
    ...
    if($actionLog->add()){
        ...
        // echo $actionLog->getLastSql();exit;
        ...
        if(strtolower($data['action_name']) == 'delete' || strtolower($data['action_name']) == 'completedelete' || strtolower($data['action_name']) == 'log_delete'){
            $actionLog->where('module_name = "%s" and action_id = "%s"', strtolower($module_name), $id)->setField('action_delete',1);
        }
        return true;
    }else{
        return false;
...
function getDateTime($model){
    ...
    M('User')->where("user_id = '%s'",$user['user_id'])->setField('last_read_time',$last_read_time);
    return true;
}
...
function getSubDepartmentTreeCode($department_id, $first=0) {
    ...
    $position_list = M('position')->where('department_id = %d', $department_id)->field('position_id,parent_id,name,department_id,description')->select();
...
function getSubPositionTreeCode($position_id, $first=0,  $type=1) {
    $string = "";
    $position_list = M('position')->where('parent_id = %d', $position_id)->field('position_id,parent_id,name,department_id,description')->select();
    //dump($position_list);
    if ($position_list) {
        if ($first) {
            if($type == 1)
...
function getSubRoleId2($role_id,$self = true){
    $all_role = M('role')->where('user_id <> 0')->field('role_id,user_id,position_id')->select();
...
function getSubRole($role_id, $role_list, $separate)
{
    ...
    //     // echo $d_role->getLastSql();exit;
    //     $position_id = $d_role->where("role.role_id = '%d' ", $role_id)->getField('position_id');
    //     //echo $d_role->getLastSql();exit;
    // }else{
    //     $position_id  = 0;
    // }
    // if(session('position_id') == 16){
    //     $position_id = 21;
    // }
    // $sub_position = getPositionSub($position_id ,false);
    // foreach($sub_position AS $position_id) {
    $user_id = session('user_id');
    if (session('position_id') == 13) {
        $sql = "SELECT u.user_id,u.role_id,u.name,u.parent_id FROM`crm_user` u,crm_role r WHERE u.user_id = r.user_id AND u.parent_id IN (SELECT user_id FROM crm_user WHERE parent_id IN (SELECT a.user_id FROM crm_user a INNER JOIN crm_user b ON a.user_id = b.user_id WHERE b.parent_id IN (SELECT user_id from `crm_user` where parent_id = \"$user_id\")))";
        $son_role = $mod->query($sql);  
    }else if (session('position_id') == 14) {
        //G('begin');
        $sql = "SELECT u.user_id,u.role_id,u.name,u.parent_id FROM`crm_user` u,crm_role r WHERE u.user_id = r.user_id AND u.parent_id IN (SELECT user_id FROM crm_user WHERE parent_id IN (SELECT a.user_id FROM crm_user a INNER JOIN crm_user b ON a.user_id = b.user_id WHERE b.parent_id = \"$user_id\"))";
        $son_role = $mod->query($sql);
        // G('end');
        // echo G('begin','end').'s';
//    }else if(session('position_id') == 1){
//        $sql = "SELECT u.user_id,u.role_id,u.name,u.parent_id FROM`crm_user` u,crm_role r WHERE u.user_id = r.user_id ";
//        $son_role = $mod->query($sql);
    }else if(session('position_id') == 15){
            // $son_role = $d_role->where("role.position_id = %d and user.parent_id = '%s'", $position_id['position_id'],session('user_id'))->select();
            // foreach ($son_role as $val){
            //     $user = $d_role->where("role.position_id = '18' and user.parent_id = '%s'",$val['user_id'])->select();
            //     foreach ($user as $val){
            //         array_push($userall,$val);
            //     }
            // }
        $myid = $mod->query("SELECT u.user_id FROM `crm_user` u,crm_role r WHERE u.user_id = r.user_id AND u.parent_id = \"$user_id\"");
        $myid2 = array();
        foreach ($myid as $val) {
            array_push($myid2,$val['user_id']); 
        }
        array_push($myid2,$user_id);
        $where['parent_id'] = array('in',$myid2);
        $son_role = M('user')->where($where)->field('user_id,role_id,name,parent_id')->select();
    }else if(session('position_id') == 16){
        $parent_id = session('parent_id');
       // echo $parent_id;exit;
        $myid = $mod->query("SELECT u.user_id FROM `crm_user` u,crm_role r WHERE u.user_id = r.user_id AND u.parent_id = \"$parent_id\"");
        $myid2 = array();
        foreach ($myid as $val) {
            array_push($myid2,$val['user_id']); 
        }
        array_push($myid2,$parent_id);
        $where['parent_id'] = array('in',$myid2);
        $son_role = M('user')->where($where)->field('user_id,role_id,name,parent_id')->select();
    }else if(session('position_id') == 21){
        $myid = $mod->query("SELECT u.user_id FROM `crm_user` u,crm_role r WHERE u.user_id = r.user_id AND u.parent_id = \"$user_id\"");
        $myid2 = array();
        foreach ($myid as $val) {
            array_push($myid2,$val['user_id']); 
        ...
        $son_role = M('user as u')->join("INNER JOIN crm_role as r on r.user_id = u.user_id")->where("u.company = '%s'",session('company'))->field('u.user_id,u.role_id,u.name as user_name,u.parent_id,u.company,r.position_id')->select();
            //dump(session('company'));
            //echo M('user as u')->getLastSql();exit;
    }
    foreach($son_role as $val){
        $array[] = array('role_id' => $val['role_id'],'user_id' => $val['user_id'], 'parent_id' => $val['parent_id']);
    }
    // }
    S('listrole',$array);
    return $array;
...
function getSubRole3($role_id, $role_list, $separate){
    $d_role = D('RoleView');
    $user_id = session('user_id');
    if (session('position_id') == 14) {
        $sql = "SELECT u.user_id,u.role_id,u.name,u.parent_id FROM`crm_user` u,crm_role r WHERE u.user_id = r.user_id AND u.parent_id IN (SELECT user_id FROM crm_user WHERE parent_id IN (SELECT a.user_id FROM crm_user a INNER JOIN crm_user b ON a.user_id = b.user_id WHERE b.parent_id = \"$user_id\")) AND r.position_id != '21'";
        $son_role = M()->query($sql);
    }else if(session('position_id') == 15 || session('position_id') == 21){
        $myid = M()->query("SELECT u.user_id FROM `crm_user` u,crm_role r WHERE u.user_id = r.user_id AND u.parent_id = \"$user_id\"");
        $myid2 = array();
        foreach ($myid as $val) {
            array_push($myid2,$val['user_id']); 
        }
        array_push($myid2,$user_id);
        $where['u.parent_id'] = array('in',$myid2);
        $son_role = M('user as u')->join("INNER JOIN crm_role as r on r.user_id = u.user_id")->where($where)->field('u.user_id,u.role_id,u.name as user_name,u.parent_id,u.company,r.position_id')->select();
    }else if(session('position_id') == 16){
        $parent_id = session('parent_id');
        $myid = M()->query("SELECT u.user_id FROM `crm_user` u,crm_role r WHERE u.user_id = r.user_id AND u.parent_id = \"$parent_id\"");
        $myid2 = array();
        foreach ($myid as $val) {
            array_push($myid2,$val['user_id']); 
        }
        ...
        $son_role = $d_role->where("role.position_id = %d and user.company = '%s'", $position_id['position_id'],session('company'))->select();
    }
    foreach($son_role as $val){
        $array[] = array('role_id' => $val['role_id'],'user_id' => $val['user_id'], 'parent_id' => $val['parent_id'], 'name' => $separate . $val['department_name'] . ' | ' . $val['role_name'],'position_id' => $val['position_id']);
    }
    return $array;
}

//产品服务请求
function getSubRole4($role_id, $role_list, $separate){
    $d_role = D('RoleView');
    $user_id = session('user_id');
    if (session('position_id') == 13) {
        $sql = "SELECT u.user_id,u.role_id,u.name,u.parent_id,r.position_id FROM`crm_user` u,crm_role r WHERE u.user_id = r.user_id AND u.parent_id IN (SELECT user_id FROM crm_user WHERE parent_id IN (SELECT a.user_id FROM crm_user a INNER JOIN crm_user b ON a.user_id = b.user_id WHERE b.parent_id = \"$user_id\")) AND r.position_id  IN ('17','16','19','22')";
        $son_role = M()->query($sql);
    }else if (session('position_id') == 14) {
        $where2['user_id'] = $user_id;
        $data=M('user')->field('user_id,parent_id')->where($where2)->find();
        if($data){
            if($data['user_id']==$data['parent_id']){
                $sql = "SELECT u.user_id,u.role_id,u.name,u.parent_id,r.position_id FROM`crm_user` u,crm_role r WHERE u.user_id = r.user_id AND u.parent_id IN (SELECT user_id FROM crm_user WHERE parent_id IN (SELECT a.user_id FROM crm_user a INNER JOIN crm_user b ON a.user_id = b.user_id WHERE b.parent_id = \"$user_id\")) AND r.position_id  IN ('17','16','19','22')";
            }else{
                $sql = "SELECT u.user_id,u.role_id,u.name,u.parent_id,r.position_id FROM`crm_user` u,crm_role r WHERE u.user_id = r.user_id AND u.parent_id IN (SELECT a.user_id FROM crm_user a INNER JOIN crm_user b ON a.user_id = b.user_id WHERE b.parent_id = \"$user_id\") AND r.position_id  IN ('17','16','19','22')";
            }
        }else{
            $sql = "";
        }
        ...
        $sql = "SELECT u.user_id,u.role_id,u.name,u.parent_id,r.position_id FROM`crm_user` u,crm_role r WHERE u.user_id = r.user_id";
        $son_role = M()->query($sql);
    }else if(session('position_id') == 15 || session('position_id') == 21){
        $myid = M()->query("SELECT u.user_id FROM `crm_user` u,crm_role r WHERE u.user_id = r.user_id AND u.parent_id = \"$user_id\"");
        $myid2 = array();
        ...
        array_push($myid2,$user_id);
        $where['u.parent_id'] = array('in',$myid2);
        $son_role = M('user as u')->join("INNER JOIN crm_role as r on r.user_id = u.user_id")->where($where)->where('r.position_id != "18"')->field('u.user_id,u.role_id,u.name as user_name,u.parent_id,u.company,r.position_id')->select();
    }else if(session('position_id') == 16){
        $parent_id = session('parent_id');
        $myid = M()->query("SELECT u.user_id FROM `crm_user` u,crm_role r WHERE u.user_id = r.user_id AND u.parent_id = \"$parent_id\"");
        $myid2 = array();
        ...
        array_push($myid2,$parent_id);
        $where['u.parent_id'] = array('in',$myid2);
        $son_role = M('user as u')->join("INNER JOIN crm_role as r on r.user_id = u.user_id")->where($where)->where('r.position_id != "20"')->field('u.user_id,u.role_id,u.name as user_name,u.parent_id,u.company,r.position_id')->select();
    }else{
        $son_role = $d_role->where("role.position_id = %d and user.company = '%s'", $position_id['position_id'],session('company'))->select();
    }
    foreach($son_role as $val){
...
function getPositionSub($position_id ,$sub = false){
    $sub_position = M('position')->where('parent_id = %d', $position_id)->field('position_id,parent_id,name,department_id,description')->select();
    //echo M('position')->getLastSql();
    //dump($sub_position);exit;
    $array = $sub_position;
    if($sub){
...
function getRoleByDepartmentId3($department_id, $sub=false){
    $id_array = array($department_id);
    $departments = M('roleDepartment')->select();
    //dump($departments);
    $where['position.department_id'] = $department_id;
    //$where['user.company'] = $_SESSION['company'];
    if(!$sub) $where['role.role_id'] = array('in', getSubRoleId3());
    $roleList = D('RoleView')->where($where)->select();
    //echo  D('RoleView')->getLastSql();
    //dump($roleList);
    foreach($departments AS $value) {
        if ($department_id == $value['parent_id']) {
            $id_array[] = $value['department_id'];
            $role_list = getRoleByDepartmentId3($value['department_id']);
            if(!empty($role_list)){
                $roleList = array_merge($roleList, $role_list);
            }
        }
...
function getRoleByDepartmentId4($department_id, $sub=false){
    $id_array = array($department_id);
    $departments = M('roleDepartment')->select();
    //dump($departments);
    $where['position.department_id'] = $department_id;
    //$where['user.company'] = $_SESSION['company'];
    if(!$sub) $where['role.role_id'] = array('in', getSubRoleId4());
    $roleList = D('RoleView')->where($where)->select();
    //echo  D('RoleView')->getLastSql();
    //dump($roleList);
    foreach($departments AS $value) {
        if ($department_id == $value['parent_id']) {
            $id_array[] = $value['department_id'];
            $role_list = getRoleByDepartmentId4($value['department_id']);
            if(!empty($role_list)){
                $roleList = array_merge($roleList, $role_list);
            }
        }
...
function getRoleByDepartmentId2($department_id, $sub=false){
    $id_array = array($department_id);
    $departments = M('roleDepartment')->select();
    //dump($departments);
    $where['position.department_id'] = $department_id;
...
function addzhuuser(){
    $where['user.company'] = $_SESSION['company'];
    $where['role.role_id'] = session('role_id');
    $roleList = D('RoleView')->where($where)->select();
    return $roleList;
}
...
//获取公司主账号的id
function getCompanyUserId($department_id, $sub=false){
    $id_array = array($department_id);
    $departments = M('roleDepartment')->select();
    //dump($departments);
    $where['position.department_id'] = $department_id;
    ...
    if(!$sub) $where['role.role_id'] = array('in', getSubRoleId());
    //dump($where);
    $roleList = D('RoleView')->where($where)->where('user.user_id = user.parent_id')->select();
    //echo  D('RoleView')->getLastSql();
    //dump($roleList);
    foreach($departments AS $value) {
        if ($department_id == $value['parent_id']) {
...
//获取公司员工
function getCompanyUsers($department_id, $role,$sub=false){
    $id_array = array($department_id);
    $departments = M('roleDepartment')->select();
    //dump($role);
    $where['position.department_id'] = $department_id;
    //$where['user.company'] = $_SESSION['company'];
    if(!$sub) $where['role.role_id'] = array('in', getSubRoleId2($role));
    //dump($where);
    $roleList = D('RoleView')->where($where)->select();
    //echo  D('RoleView')->getLastSql();exit;
    //dump($roleList);
    foreach($departments AS $value) {
        if ($department_id == $value['parent_id']) {
            $id_array[] = $value['department_id'];
...
function getUserByRoleId($role_id){
    $role = D('RoleView')->where("role.role_id = %d and user.company = '%s'", $role_id,$_SESSION['company'])->find();
    //echo D('RoleView')->getLastSql();
    return $role;
}

function getUserByRoleId2($role_id){
    $role = D('RoleView')->where("role.role_id = %d", $role_id)->cache(true,7200,'file')->find();
    //echo D('RoleView')->getLastSql();
    return $role;
}

//获取区域代理商
function getAreaAgent($user){
    if($user['position_id'] == 15 || $user['position_id'] == 21){
        $agent = M('user')->field('user_id,name')->where('user_id = "%s"',$user['user_id'])->cache(true)->find();
        //echo M('user')->getLastSql();
    }else if($user['position_id'] == 16 || $user['position_id'] == 19 ||$user['position_id'] == 17 || $user['position_id'] == 22){
        $agent = M('user')->field('user_id,name')->where('user_id = "%s"',$user['user_parent_id'])->cache(true)->find();
        //echo M('user')->getLastSql();
    }else if($user['position_id'] == 18 ||$user['position_id'] == 20){
        $chiefId = M('user')->field('parent_id')->where('user_id = "%s"',$user['user_parent_id'])->cache(true)->find();
        $agent = M('user')->field('user_id,name')->where('user_id = "%s"',$chiefId)->cache(true)->find();
    }
...
//获取总代理
function getGenerAgent($user){
    if($user['position_id'] == 15 || $user['position_id'] == 21){
        $generagent = M('user')->field('user_id,name')->where('user_id = "%s"',$user['user_parent_id'])->cache(true)->find();
        //echo M('user')->getLastSql();
    }else if($user['position_id'] == 16 || $user['position_id'] == 19 ||$user['position_id'] == 17 || $user['position_id'] == 22){
        $chiefId = M('user')->field('parent_id')->where('user_id = "%s"',$user['user_parent_id'])->cache(true)->find();
        $generagent = M('user')->field('user_id,name')->where('user_id = "%s"',$chiefId)->cache(true)->find();
        //echo M('user')->getLastSql();
    }else if($user['position_id'] == 18 ||$user['position_id'] == 20){
        $chiefId = M('user')->where('user_id = "%s"',M('user')->field('parent_id')->where('user_id = "%s"',$user['user_parent_id'])->find())->getField('parent_id');
        $generagent = M('user')->field('user_id,name')->where('user_id = "%s"',$chiefId)->cache(true)->find();
    }
    return $generagent;
}
...
function field_list_html($type="add",$module="",$d_module=array()){
    ...
    foreach($field_list as $k=>$v){
        ...
        if($d_module['customer_id']){
            $customer_id = ($d_module['customer_id']);
        }else{
            $customer_id = ($_GET['customer_id']);
        }

        if($customer_id){
            $customer = M('customer')->where("customer_id = '%s'", $customer_id)->find();
...
if ($v['field'] == 'create_time' || $v['field'] == 'update_time') {
    ...
    $customer_id = ($_GET['customer_id']);
    if($v['field'] == 'name' && $customer_id && $module == 'customer') $value=M('customer')->where("customer_id = '%s'", $customer_id)->getField('name');
...
function check_permission($module_id, $module, $permission_field='owner_role_id'){
    $role_id = (session('role_id'));
    $owner_role_id = M($module)->where($module."_id = '%s'", $module_id)->getField($permission_field);
...
function getLastedImg(){
    ...
    if(!$imgsrc){
        $imgsrc = M('User')->where("user_id = '%s'", (session('user_id')))->cache(true)->getField('img');
    }
    if($imgsrc){
        return $imgsrc;
    }else{
...
function getUserInfo($userId){
    $userInfo = M('user')->where('user_id = "%s"',$userId)->field('user_id,name,role_id,company,parent_id')->find();
    return $userInfo;
}
...
function getUserInfo2($userId){
    $userInfo = M('user')->where('user_id = "%s"',$userId)->getField('name');
    return $userInfo;
}
...
function orderAna($order){
    ...
    $html = file_get_contents(C('URL').'/manager/order/getOrderStatisticByUserId', false, $context);
...
function productAna($pro){
    ...
    $context = stream_context_create($opts);
    $html = file_get_contents(C('URL').'/manager/serviceGoods/queryCrmCount', false, $context);
    $html = json_decode($html,true);
    $pronum = $html['data'];
    return $pronum;
}
 ...
 * @return String
 */
function encode($string = '', $skey = 'luiChi') {
 ...
 * @return String
 */
function decode($string = '', $skey = 'luiChi') {
...
function getuser($type,$id){
    $user = S('commomuser');
    if(!$user){
        S('commomuser',M('user as u')->join("INNER JOIN crm_role as r on r.user_id = u.user_id")->field('u.user_id,u.role_id,u.name as user_name,u.parent_id,u.company,r.position_id')->select(),3600,'File');
    }
    ...
    $sans = array();
    $fours = array();
    if(session('position_id') == 14 && $type==''){
        //二级
        foreach ($user as $two){
            if($two['parent_id'] == session('user_id') && ($two['position_id'] == 15|| $two['position_id'] == 21)){
                array_push($twos,$two);
                array_push($all,$two);
            }
        }
        ...
        //四级
        foreach ($user as $two){
            foreach ($sans as $four){
                if($four['user_id'] == $two['parent_id']){
                    array_push($fours,$two);
                    array_push($all,$two);
                }
            }
        }
    ...
    }else if((session('position_id') == 15&& $type=='') || (session('position_id') == 21 && $type=='')){
        //二级
        foreach ($user as $two){
            if($two['user_id'] == session('user_id')){
                array_push($twos,$two);
                array_push($all,$two);
            }
        }
    ...
    }else if(session('position_id') == 16 && $type == ''){
        //二级
        foreach ($user as $two){
            if($two['user_id'] == session('parent_id')){
                array_push($twos,$two);
                //array_push($all,$two);
            }
        }
    ...
    }else if($type == 'tradeagent' || $type == 'shopsagent'){
        //二级
        foreach ($user as $two){
            if($two['user_id'] == $id){
                array_push($twos,$two);
        ...
        $userid = M('user')->field('parent_id')->where('user_id = "%s"',$id)->find();
        //二级
        foreach ($user as $two){
            if($two['user_id'] == $userid['parent_id']){
                array_push($twos,$two);
                //array_push($all,$two);
            }
        }
...
function getuser2($type,$id){
    $user = S('commomuser');
    if(!$user){
        S('commomuser',M('user as u')->join("INNER JOIN crm_role as r on r.user_id = u.user_id")->field('u.user_id,u.role_id,u.name as user_name,u.parent_id,u.company,r.position_id')->order('u.company desc')->select(),3600,'File');
    }
    //echo M('user as u')->getLastSql();
    $twos = array();
    $all = array();
    ...
    if(session('position_id') == 14){
        //二级
        foreach ($user as $two){
            if($two['parent_id'] == session('user_id') && $two['position_id'] == 15){
                array_push($twos,$two);
                array_push($all,$two);
            }
        }
...
function getuserid(){
    $user = getSubRole4(session('role_id'));
    $users = array();
    foreach ($user as $val){
        if($val['position_id'] == 13 ||$val['position_id'] == 14 ||$val['position_id'] == 16 || $val['position_id'] == 17 || $val['position_id'] == 18 || $val['position_id'] == 19 || $val['position_id'] == 20 || $val['position_id'] == 22){
            array_push($users,$val['user_id']);
        }
    }
    if(session('other') == 2){
    	$userList = M('user')->where(['company' => session('company')])->select();
    	$user2 = [];
    	foreach ($userList as $val){
    		array_push($user2,$val['user_id']);
    	}
    	$users = array_diff($users,$user2);
    }
    $userid = implode(",",$users);
    return $userid;
}
//查找自己员工和自己的id
function getserviceUserId(){
	$userList = M('user')->where(['company' => session('company')])->select();
	$users = array();
	foreach ($userList as $val){
		array_push($users,$val['user_id']);
...
function getuser3($type,$id,$unfour){
    $user = S('commomuser');
    if(!$user){
        S('commomuser',M('user as u')->join("INNER JOIN crm_role as r on r.user_id = u.user_id")->field('u.user_id,u.role_id,u.name as user_name,u.parent_id,u.company,r.position_id')->order('u.company desc')->select(),3600,'File');
    }
...
//统计使用
function getTJ($position,$type=true)
{
    $d_role = D('RoleView');
    $mod = new Model();
    $user_id = session('user_id');
    if(session('position_id')==1) {
        $user_id= '9BCA0777DE557245DD391FED16CDF013';
    }
    if ($position['position_id'] == 13) {
        if (!$type) {
            $where['u.parent_id'] = array(in, $position['user_id']);
            $userId = M('user as u')->join('INNER JOIN crm_user as r on r.user_id = u.user_id')->where($where)->field('u.user_id')->select();
            $mapId =  array();
            foreach ($userId as $val){
                array_push($mapId,$val['user_id']);
            }
            $where1['u.parent_id'] = array(in, $mapId);
            $son_role = M('user as u')->join('INNER JOIN crm_role as r on r.user_id = u.user_id')->where($where1)->where("r.position_id in('16','17','19','22')")->field('u.user_id,u.role_id,u.name as user_name,u.parent_id,u.company,r.position_id')->select();          
        }        
    }elseif ($position['position_id'] == 14) {
        if ($type) {
            $sql = "SELECT u.user_id,u.role_id,u.name,u.parent_id FROM`crm_user` u,crm_role r WHERE u.user_id = r.user_id AND u.parent_id IN (SELECT user_id FROM crm_user WHERE parent_id IN (SELECT a.user_id FROM crm_user a INNER JOIN crm_user b ON a.user_id = b.user_id WHERE b.parent_id = \"$user_id\")) AND r.position_id IN('16','17','18','19','20','22')";
        } else {
            if(session('position_id')==14){
                $sql = "SELECT u.user_id,u.role_id,u.name,u.parent_id FROM`crm_user` u,crm_role r WHERE u.user_id = r.user_id AND u.parent_id IN (SELECT a.user_id FROM crm_user a INNER JOIN crm_user b ON a.user_id = b.user_id WHERE b.parent_id = \"$user_id\") AND r.position_id IN('16','17','19','22')";
            }else{
                $sql = "SELECT u.user_id,u.role_id,u.name,u.parent_id FROM`crm_user` u,crm_role r WHERE u.user_id = r.user_id AND u.parent_id IN (SELECT user_id FROM crm_user WHERE parent_id IN (SELECT a.user_id FROM crm_user a INNER JOIN crm_user b ON a.user_id = b.user_id WHERE b.parent_id = \"$user_id\")) AND r.position_id IN('16','17','19','22')";
            }
        }
        $son_role = $mod->query($sql);
    }elseif (session('position_id')==1){
        $sql = "SELECT u.user_id,u.role_id,u.name,u.parent_id FROM`crm_user` u,crm_role r WHERE u.user_id = r.user_id AND u.parent_id IN (SELECT user_id FROM crm_user WHERE parent_id IN (SELECT a.user_id FROM crm_user a INNER JOIN crm_user b ON a.user_id = b.user_id WHERE b.parent_id = \"$user_id\")) AND r.position_id IN('16','17','18','19','20','22')";
        $son_role = $mod->query($sql);
    }else if($position['position_id'] == '15'  || $position['position_id'] == '21'){
        if($position['position_id'] == '15'  || $position['position_id'] == '21'){
        ...
        if($type){
            ...
            $son_role = M('user')->where($where)->field('user_id,role_id,name,parent_id')->group('company')->select();
         }
 
    }else if($position['position_id'] == '16'){
        $parent_id = M('user')->where("user_id = '%s'",$position['user_id'])->getField('parent_id');
        $myid = $mod->query("SELECT u.user_id FROM `crm_user` u,crm_role r WHERE u.user_id = r.user_id AND u.parent_id = \"$parent_id\"");
        $myid2 = array();
        foreach ($myid as $val) {
            array_push($myid2,$val['user_id']); 
        ...
        $where['parent_id'] = array('in',$myid2);
        $son_role = M('user')->where($where)->field('user_id,role_id,name,parent_id')->select();
    }else if($position['position_id'] == '17' || $position['position_id'] == '19' || $position['position_id'] == '22'){
        // $son_role = M('user as u')->join("INNER JOIN crm_role as r on r.user_id = u.user_id")->where("u.company = '%s'",M('user')->where("user_id = '%s'",$position['user_id'])->getField('company'))->field('u.user_id,u.role_id,u.name as user_name,u.parent_id,u.company,r.position_id')->select();
        $son_role = M('user as u')->join("INNER JOIN crm_role as r on r.user_id = u.user_id")->where("u.user_id = '%s'",$position['user_id'])->field('u.user_id,u.role_id,u.name as user_name,u.parent_id,u.company,r.position_id')->select();
    }else{
        if($type){
            $son_role = M('user as u')->join("INNER JOIN crm_role as r on r.user_id = u.user_id")->where("u.company = '%s'",session('company'))->field('u.user_id,u.role_id,u.name as user_name,u.parent_id,u.company,r.position_id')->select();
        }else{
            $son_role = M('user')->where("user_id = '%s'",$_SESSION['user_id'])->field('user_id,role_id,parent_id,name as user_name')->select();
        }
        
    }
    foreach($son_role as $val){
        $array[] = array('role_id' => $val['role_id'],'user_id' => $val['user_id'], 'parent_id' => $val['parent_id'],'user_name'=>$val['user_name']);
    }
    return $array;
}
