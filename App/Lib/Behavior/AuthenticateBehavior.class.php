...
class AuthenticateBehavior extends Behavior {
	...
	public function run(&$params) {
		...
		//dump($_SESSION);
		//dump($_COOKIE);
		if(!session('?user_id') && (cookie('user_id')) != 0 && trim(cookie('name')) != '' && trim(cookie('salt_code')) != ''){
			$user = M('user')->where(array('user_id' => (cookie('user_id'))))->find();
		...
		if (in_array($a, $permission)) {
			...
			if (in_array($a, $allow)) {
				...
				//dump($url);
                if(session('position_id')!=22) {
                    $ask_per = M('permission')->where('url = "%s" and position_id = %s', $url, session('position_id'))->field('role_id,position_id,url,description,permission_id')->find();
                }else{
                    $ask_per = M('permission')->where('url = "%s" and position_id = %s', $url, 17)->field('role_id,position_id,url,description,permission_id')->find();
                }
                //echo M('permission')->getLastSql();
