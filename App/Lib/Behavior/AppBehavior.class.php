	...
	public function run(&$params) {
		...
		if(defined('APP_TYPE') && APP_TYPE == 'Mobile'){
		 ...
		 if (!file_exists( CONF_PATH.'install.lock') && MODULE_NAME != 'Install') {
		 	...
		 	if (!F('smtp')) {
		 		$value = M('Config')->where('name = "smtp"')->cache(true)->getField('value');
		 		F('smtp',unserialize($value));
		 	}
		 	C('smtp', F('smtp'));
		 	if (!F('defaultinfo')) {
		 		$value = M('Config')->where('name = "defaultinfo"')->cache(true)->getField('value');
		 		F('defaultinfo',unserialize($value));
		 	}
		 	C('defaultinfo', F('defaultinfo'));
		 	if (!F('sms') && $value = M('Config')->where('name = "sms"')->cache(true)->getField('value')) {
