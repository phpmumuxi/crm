...
class DynamicAction extends Action{
	...
	public function index(){
		// G('begin');
		$params = S('params'.session('user_id'));
		$raction_log = S('action_log'.session('user_id'));
		$revent_count = S('revent_count'.session('user_id'));
		$rtask_count = S('task_count'.session('user_id'));
		$rshow = S('show'.session('user_id'));
...
//			//new 总代理 代理查看自己公司的信息
			...
			foreach($action_log as $k=>$v){
				...
				$action_log[$k]['sign_img'] = $m_sign_img ->where('sign_id = "%s"',$v['action_id'])->select();
				//echo $m_sign_img->getLastSql();
				$m_module_name = M($module_name);
				$pk_id = $m_module_name->getPk();
				$object_module = $m_module_name->cache(true,60)->where("$pk_id = '%s'", $v['action_id'])->find();
				//echo $m_module_name->getLastSql();
				//dump($object_module);
				$name = $object_module['name'];
				if($object_module['number']){
				...
				//dump($action_log[$k]['dynamic']);
				//查评论
				//如果是评论日志，则从操作日志表中根据action_id查找，否则根据操作日志表的log_id查找
				if($v['module_name'] == 'log'){
					$comment = $m_comment->where('module = "%s" and module_id = "%s"', 'log', $v['action_id'])->order('create_time desc')->limit(0,5)->select();
					$comment_count = $m_comment->where('module = "%s" and module_id = "%s"', 'log', $v['action_id'])->order('create_time desc')->count();
				}else{
			...
			S('params'.session('user_id'),$params,1,'File');
			$this->parameter = implode('&', $params);

			S('action_log'.session('user_id'),$action_log,1,'File');
			$this->actionLog = $action_log;

			S('event_count'.session('user_id'),$event_count,1,'File');
			$this->event_count = $event_count;

			S('task_count'.session('user_id'),$task_count,1,'File');
			$this->task_count = $task_count;

			$this->userInfo = getUserByRoleId(session('role_id'));
			$this->department = getSubDepartment2(0,M('roleDepartment')->cache(true)->where('department_id != 3')->select());

			$show = $Page->show();
			S('show'.session('user_id'),$show,1,'File');
			$this->page = $show;
		}else{
			// echo "缓存";
//			G('begin');
...
//			echo G('begin','end',6).'s';
			$this->parameter = implode('&', $params);
			$this->actionLog = $raction_log;
			$this->event_count = $revent_count;
			$this->task_count = $rtask_count;
			$this->userInfo = getUserByRoleId(session('role_id'));
			$this->department = getSubDepartment2(0,M('roleDepartment')->cache(true)->where('department_id != 3')->select());
			$this->page = $rshow;
		...
		$managerHtml = send_post(C('URL').'/manager/seUserHouseManager/queryManagerInfo',['userId' =>session("user_id")]);
		$managerHtml = json_decode(json_encode($managerHtml),TRUE);
		//var_dump($managerHtml);exit;
		if($managerHtml["success"])
			$this->assign("managerLog",$managerHtml["data"]["isHouseManager"]);
	...
	public function more(){
		...
		if($actionLog['module_name'] == 'log'){
			$comment = $m_comment->where('module = "%s" and module_id = "%s"', 'log', $actionLog['action_id'])->order('create_time desc')->limit(5,1000)->select();
		}else{
			$comment = $m_comment->where('module = "%s" and module_id = "%s"', 'dynamic', $actionLog['log_id'])->order('create_time desc')->limit(5,1000)->select();
		}
		foreach($comment as $key=>$val){
			$comment[$key]['comment_role'] = getUserByRoleId($val['creator_role_id']);
	...
	public function getTransLocation(){
		...
		$url = "http://api.map.baidu.com/geoconv/v1/?coords=$x,$y&from=3&to=5&ak=YfrMk3eIKhguMXWprcGSHyr6VSY9V6MO";
