...
class ContactsAction extends Action {
	...
	public function _initialize(){
		$action = array(
			'permission'=>array(),
			'allow'=>array('checklistdialog','getContactsList', 'revert', 'mdelete','radiolistdialog','changedialog','add_dialog','qrcode','changetofirstcontact','completedelete')
		);
	...
	public function add(){
		if ($_GET['r'] && $_GET['module'] && $_GET['id']) {
			...
			$contacts->company = $_SESSION['company'];
	...
	public function edit(){
		...
		$customer_id = $rContactsCustomer->where("contacts_id = '%s'", $contacts_id)->getField('customer_id');
		...
		if ($this->isPost()) {
			...
			if (!empty($_POST['customer_id'])) {
				if (empty($customer_id)) {
					...
					$rContactsCustomer -> where("contacts_id = '%s'" , $_POST['contacts_id']) -> setField('customer_id',$_POST['customer_id']);
				}	
			}else{
	...
	public function index(){
		...
		switch ($by) {
			...
			case 'week' : $where['create_time'] =  array('gt',(strtotime(date('Y-m-d', time())) - (7) * 86400)); break;
		...
		if(trim($_GET['act']) == 'excel'){
			...
			$this->role_list = $d_role_view->where("role.role_id in (%s) and user.company = '%s'", implode(',', $below_ids),$_SESSION['company'])->select();
	...
	public function completeDelete(){
		...
		//echo M('customer')->getLastSql();exit;
		...
		if ($_POST['contacts_id']) {
			if (!session('?admin')) {
				foreach ($_POST['contacts_id'] as $value) {
					$customer_id = $rContactsCustomer->where("contacts_id = '%s'", $value)->getField('customer_id');
			...
			if (is_array($contacts)) {
				//检查权限
				$customer_id = $rContactsCustomer->where("contacts_id = '%s'", $contacts_id)->getField('customer_id');
				//echo $rContactsCustomer->getLastSql();exit;
				if (session('?admin') || in_array($customer_id, $customer_idArr)) {
					if($m_contacts->where("contacts_id = '%s'", $contacts_id)->delete()){
						foreach ($r_module as $key2=>$value2) {
							if(!is_int($key2)){
	...
	public function mDelete(){
		if($_GET['r'] && $_GET['id'] && $_GET['module_id']){
			$m_r = M($_GET['r']);
			if($m_r->where("contacts_id = %d and customer_id = '%s'", $_GET['id'], $_GET['module_id'])->delete()){
	...
	public function getContactsList(){
		...
		//获取下级和自己的客户列表,搜索
		$contactsList = D('Contacts')->where(array('creator_role_id'=>array('in',$idArray),'is_deleted'=>array('eq', 0)))->select();
	...
	public function excelExport($contactsList){
		...
		foreach ($list as $k => $v) {
			...
			$customer_id = M('rContactsCustomer')->where("contacts_id = '%s'", $v['contacts_id'])->getField('customer_id');
			$customer_name = M('customer')->where("customer_id = '%s'" ,$customer_id)->getField('name');
	...
	public function radioListDialog(){
		...
		if($id){
			$contacts_id = $rcc->where("customer_id = '%s'", $id)->getField('contacts_id', true);
			$where['contacts_id'] = array('in', implode(',', $contacts_id));
			$this->customer_id = $id;
		}

        $where['customer.company'] = $_SESSION['company'];
		$list = $d_contacts->where($where)->order('create_time desc')->limit(10)->select();
		//echo $d_contacts->getLastSql();
		...
		foreach ($list as $k=>$value) {
			...
			$list[$k]['customer'] = M('customer')->where("customer_id = '%s'", $customer_id)->find();
		}
	...
	public function changeDialog(){
		if($this->isAjax()){
			...
			if($_REQUEST['customer_id'] != 0){
				$contacts_id = M('RContactsCustomer')->where("customer_id = '%s'", $_REQUEST['customer_id'])->getField('contacts_id', true);
				$where['contacts_id'] = array('in', implode(',', $contacts_id));
			...
			$where['customer.company'] = $_SESSION['company'];
			$contactsList = $d_contacts->where($where)->order('create_time desc')->page($p.',10')->select();
			//echo  $d_contacts->getLastSql();
			$count = $d_contacts->where($where)->count();
			if(!$_REQUEST['is_check']){
				foreach ($contactsList as $k => $v) {
					if($customer_id = M('rContactsCustomer')->where('contacts_id = %d', $v['contacts_id'])->getField('customer_id')){
						$contactsList[$k]['customer'] = $d_contacts->where("customer.customer_id = %s'" ,$customer_id)->find();
					}
				}
			}
...
//echo '<pre>';print_r($data);echo '</pre>'; die();
	...
	public function qrcode(){
		...
		if($contacts = M('Contacts')->where('contacts_id = %d', $contacts_id)->find()){
			...
			$contacts['customer'] = M('Customer')->where("customer_id = '%s'", $customer_id)->getField('name');
			$qrOpt = '';
			$qrOpt = "BEGIN:VCARD\nVERSION:3.0\n";
