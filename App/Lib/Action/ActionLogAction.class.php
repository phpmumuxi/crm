...
class ActionLogAction extends Action{
	...
	public function index(){
		...
		foreach($list as $k=>$v){
			...
			$name = $m_module_name->where("$id = '%s'", $v['action_id'])->getField('name');
			$number = $m_module_name->where("$id = '%s'", $v['action_id'])->getField('number');
			//dump($number);
			if($number){
				$name = $number;
			}
			if(empty($name)){
				$name = $m_module_name->where("$id = '%s'", $v['action_id'])->getField('subject');
			}
