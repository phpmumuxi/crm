...
class BusinessAction extends Action{
    ...
    public function _initialize(){
        $action = array(
            'permission'=>array(''),
            'allow'=>array('close','analytics','getaddchartbyroleid','changecontent','getownchartbyroleid','advance','validate','check','revert','getsalesfunnel','getcurrentstatus','completedelete')
        );
    ...
    public function check() {
        ...
        if ($this->isAjax()) {
            ...
            $name_list = $m_business->where("company = '%s'",$_SESSION['company'])->getField('name', true);
            $seach_array = array();
        ...
        if($this->isPost()){
            ...
            if($m_business->create()){
                if($m_business_data->create()!==false){
                    ...
                    $m_business->company = $_SESSION['company'];
                    if($business_id = $m_business->add()){
                        ...
                        if($m_business_data->add()){
                            ...
                            if(intval($_POST['status_id']) == 100){
                                M('Customer')->where("customer_id = '%s'", ($_POST['customer_id']))->setField('is_locked',1);
                            }
    ...
    public function view(){
        ...
        $business['customer'] = M('Customer')->where("customer_id = '%s'", $business['customer_id'])->find();
        $business['contacts'] = M('contacts')->where("contacts_id = '%s' and is_deleted=0", $business['contacts_id'])->find();
        $business['owner'] = getUserByRoleId($business['owner_role_id']);
    ...
    public function index(){
            ...
            if ($this->_request('state')){
                ...
                if($field =="customer_id"){
                   // $c_where['name'] = array('like','%'.$search.'%');
                    $c_where['customer_id'] = array('like','%'.$search.'%');
        ...
        //dump($where);
        //添加按照公司查询
        //$where['business.company'] = $_SESSION['company'];
        $list = $d_v_business->where($where)->order($order)->page($p.','.$listrows)->select();
        //echo  $d_v_business->getLastSql();
    ...
    public function changeContent(){
        if($this->isAjax()){
            ...
            $where['is_deleted'] = 0;
            //print_r(getSubRoleId());
            $where['owner_role_id'] = array('in',implode(',', getSubRoleId()));
           // $where['owner_role_id'] = $_REQUEST['search'];
            $where['business.company'] = $_SESSION['company'];
            if ($_REQUEST["field"]) {
                if (trim($_REQUEST['field']) == "all") {
        ...
        foreach ($list as $k => $v) {
            ...
            foreach($field_info as $field){
                if($field['form_type'] == 'datetime'){
                    ...
                    $customer = $m_customer->where("customer_id = '%s'",$v['customer_id'])->find();
    ...
    public function advance(){
        if($this->isPost()){
            ...
            if(intval($_POST['status_id']) == 100){
                M('Customer')->where("customer_id = '%s'", $business['customer_id'])->setField('is_locked',1);
            }

            if($m_business->where('business_id = %d', $id)->save($data2)){
                $m_business_data->where('business_id = %d', $id)->save($data3);
                M('customer')->where("customer_id = '%s'",$business['customer_id'])->setField('update_time',time());
