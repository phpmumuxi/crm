...
class CustomerAction extends Action {
    ...
    public function _initialize(){
        $action = array(
            'permission'=>array(),
            'allow'=>array('getcustomerlist','analytics', 'validate','check', 'remove', 'fenpei', 'revert','changecontent','customerlock','check_customer_limit','excelimportdownload','search','listDialog','getcustomeroriginal','batchclose','batchfocus','close_share','share','getcurrentstatus','sync','completedelete','agentview')
        );
        B('Authenticate', $action);

//		$ip = gethostbyname('www.clubluichi.com');
//		echo $ip;
        //数据同步()
        //请求api接口  把接收到的客户数据同步到数据库中 (1.用户 公司)
        //同步位置(1客户)
    ...
    */
    ...
    public function check(){
        ...
        if ($this->isAjax()) {
            ...
            $name_list = $m_customer->where("company = '%s'",$_SESSION['company'])->getField('name', true);
            $seach_array = array();
    ...
    public function validate() {
        if($this->isAjax()){
            ...
            if($this->_request('id','',0)){
                $where[$m_customer->getpk()] = array('neq',$this->_request('id','',0));
            }
    ...
    public function remove(){
        if($this->isPost()){
            ...
            foreach($customerIds as $v){
                $role = M('customer')->where('customer_id = "%s"',$v)->getField('owner_role_id');
                if(session('company') !== M('user')->where("role_id = '%s'",$role)->getField('company')){
                    alert('error', "部分客户没有权限",$_SERVER['HTTP_REFERER']);
                }

            }
            $haha = array('customer_id'=>array('in',$_POST['customer_id']));
            $haha['is_locked']= 1;
            // print_r($haha);
            $lock_names = $m_customer->where($haha)->getField('name',true);
            // echo $m_customer->getLastSql();
            // exit;
            if($lock_names){
                $customers = implode(' , ',$lock_names);
                alert('error','客户('.$customers.')已被锁定，不能放入客户池！',$_SERVER['HTTP_REFERER']);
            }
            if($m_customer->where(array('customer_id'=>array('in',$_POST['customer_id'])))->setField('owner_role_id',0)){
    ...
    public function receive(){
        ...
        if($this->isGet()){
            ...
            if($customer_record_count < $customer_limit_counts){
                $contacts = M('rContactsCustomer')->where("customer_id = '%s'", $customer_id)->select();
                foreach($contacts as $k=>$v ){
                    M('contacts')->where('contacts_id = %d', $v['contacts_id'])->setField('owner_role_id',$owner_role_id);
                }
                if($m_customer->where("customer_id = '%s'", $customer_id)->save($data)){
            ...
            if(is_array($customer_ids)){
                ...
                $where['owner_role_id'] = array('gt',0);
                $contacts = M('rContactsCustomer')->where("'customer_id = '%s", $customer_ids)->select();
                foreach($contacts as $k=>$v ){
                    M('contacts')->where('contacts_id = %d', $v['contacts_id'])->setField('owner_role_id',$owner_role_id);
                }
                $updated_owner = $m_customer->where($where)->save($data);
                ...
                if($updated_owner || $updated_time){
                    $customer = $m_customer->where("customer_id = '%s'", ($customer_ids))->find();
                    $title=L('you_have_new_customer');
            ...
            if($_GET['by'] == 'put'){
                if($customer_id){
                    ...
                    if($customer['is_locked'] == 0){
                        if(M('customer')->where("customer_id = '%s'", $customer_id)->setField('owner_role_id',0)){
...
//高级搜索
    public function search(){
        ...
        switch ($by) {
            ...
            case 'month' : $where['create_time'] = array('gt',strtotime(date('Y-m-d', time()))); break;
        ...
        if(trim($_GET['act'] == 'sms')){
            ...
            if($by == 'deleted') {
                ...
                foreach ($list as $k => $v) {
                    ...
                    $list[$k]["owner"] = D('RoleView')->where('role.role_id = %d', $v['owner_role_id'])->find();
                    $list[$k]["creator"] = D('RoleView')->where('role.role_id = %d', $v['creator_role_id'])->find();
                    $days =  M('Customer')->where("customer_id = '%s'", $v['customer_id'])->getField('update_time');
    ...
    public function changeContent(){
        if($this->isAjax()){
            ...
            $p = !$_REQUEST['p']||$_REQUEST['p']<=0 ? 1 : ($_REQUEST['p']);
            $list = $m_customer->where($where)->order('create_time desc')->page($p.',10')->select();
            //echo $m_customer->getLastSql();exit;
            foreach($list as $k => $v){
...
//			echo M('Fields')->getLastSql();exit;
            //判断文本输入类型
            foreach ($field_list as $v){
                //dump($v['form_type']);
                switch($v['form_type']) {
                    //array_filter  用回调函数过滤数组中的元素 默认过滤空 0 false
                    case 'address':
                        $a = array_filter($_POST[$v['field']]);
                        //chr返回string chr(10)换行
                        //dump($a);
            ...
            if($m_customer->create()){
                if($m_customer_data->create()!==false){
                    ...
                    $m_customer->company = $_SESSION['company'];
                    //dump($customer_id);
                    // dump($_POST);exit;
                    if(!$m_customer->add()){
                        //echo $m_customer->getLastSql();exit;
                    ...
                    $m_customer_data->add();
                    //echo $m_customer_data->getLastSql();exit;
                    if ($_POST['leads_id']) {
                    ...
                    if ($contacts_id && $customer_id) {
                        ...
                        //echo M('RContactsCustomer')->getLastSql();exit;
                    }
                    if(($_POST['create_business1']) == 1 || ($_POST['create_business1']) == 1){
            ...
            if(($_GET['leads_id'])){
                $leads = D('LeadsView')->where('leads.leads_id = %d', ($_GET['leads_id']))->find();
                $this->leads = $leads;
            }
    ...
    public function delete(){
        ...
        if ($this->isPost()) {
            ...
            if ('' == $customer_ids) {
                ...
                if($m_customer->where(array('customer_id'=>array('in',$_POST['customer_id'])))->setField($data)){
                    // echo $m_customer->getLastSql();exit;
                    //记录操作记录
            ...
            $customer = $m_customer->where("customer_id = '%s'", $_GET['id'])->find();
            //($customer);exit;
            if (is_array($customer)) {
                if($customer['owner_role_id'] == session('role_id') || session('?admin')){
                    $data = array('is_deleted'=>1, 'delete_role_id'=>session('role_id'), 'delete_time'=>time());

                    if($m_customer->where("customer_id = '%s'", $_GET['id'])->setField($data)){
                        actionLog($_GET['id']);
                        //判断客户是否属于客户池
                        $outdays = M('config') -> where('name="customer_outdays"')->getField('value');
                        $outdate = empty($outdays) ? time() : time()-86400*$outdays;
    ...
    public function completeDelete() {
        ...
        if ($this->isPost()) {
            $customer_ids = stripcslashes(is_array($_POST['customer_id']) ? implode("','", $_POST['customer_id']) : '');
            //print_r(stripcslashes($customer_ids));
            ...
            if ('' == $customer_ids) {
                ...
                if($m_customer->where(array('customer_id'=>array('in',$_POST['customer_id'])))->delete()){
                    //echo $m_customer->getLastSql();exit;
                    //M('CustomerDate')->where("customer_id in ('%s')", $customer_ids)->delete();
                    M('CustomerDate')->where(array('customer_id'=>array('in',$_POST['customer_id'])))->delete();

                    foreach ($_POST['customer_id'] as $value) {
                        foreach ($r_module as $key2=>$value2) {
                            $module_ids = M($value2)->where("customer_id = '%s'", $value)->getField($key2 . '_id', true);
                            M($value2)->where("customer_id = '%s'", $value)->delete();
                            if(!is_int($key2)){
            ...
            if (is_array($customer)) {
                if($customer['owner_role_id'] == session('role_id') || session('?admin')){
                    if($m_customer->where("customer_id = '%s'", $_GET['id'])->delete()){
                        //actionLog($_GET['id']);
                        M('CustomerDate')->where("customer_id = '%s'", $_GET['id'])->delete();
                        foreach ($r_module as $key2=>$value2) {
                            $module_ids = M($value2)->where("customer_id = '%s'", $_GET['id'])->getField($key2 . '_id', true);
                            M($value2)->where("customer_id = '%s'", $_GET['id']) -> delete();
                            if(!is_int($key2)){
                                M($key2)->where($key2 . '_id in (%s)', implode(',', $module_ids))->delete();
                            }
                        }
    ...
    public function edit(){
        if(!check_permission(($this->_request('id')), 'customer')) $this->error(L('HAVE NOT PRIVILEGES'));
        $customer = D('CustomerView')->where("customer.customer_id = '%s'",$this->_request('id'))->find();
        // echo D('CustomerView')->getLastSql();
        if(session('company') !== M('user')->where("role_id = '%d'",$customer['owner_role_id'])->getField('company')){
            alert('error', '没有权限',$_SERVER['HTTP_REFERER']);
        }
        ...
        //dump($customer['contacts_name']);
        //查找商户字段
        $field_list = M('Fields')->where('model = "customer"')->order('order_id')->select();
        //dump($customer['$field_list']);
        if($this->isPost()){
            $m_customer = D('Customer');
            $m_customer_data = D('CustomerData');//客户附表信息
            foreach ($field_list as $v){
                switch($v['form_type']) {
                    case 'address':
            ...
            if($customer['is_locked'] == '1' && $_POST['owner_role_id'] == ''){
                alert('error','锁定用户不能放入客户池!',$_SERVER['HTTP_REFERER']);
            }
            // dump($_POST);exit;
            if($m_customer->create()){
                if($m_customer_data->create()!==false){
                    ...
                    if($a !== false && $b !== false){
                        if($_POST['contacts_id'] && ($_POST['contacts_id'] != $customer['contacts_id'])){
                            $rcc['contacts_id'] = ($_POST['contacts_id']);
    ...
    public function index(){
        $d_v_customer = D('CustomerView');//自定义module类
        $by = isset($_GET['by']) ? trim($_GET['by']) : '';
        //获取权限id
        //默认为当前人下属roleid  返回值$below_ids (getSubRoleId)
        $all_ids = $below_ids = getSubRoleId(true);
        //$all_ids = getSubRoleId(true);
        //dump($all_ids);
        $outdays = M('config') -> where('name="customer_outdays"')->getField('value');//线索超出30天数放入客户池
        //$outdays 空现在时间 非空前一个月时间
        $outdate = empty($outdays) ? time() : time()-86400*$outdays;
        //echo time();
        //dump(date('Y-m-d',$outdate));
        $where = array();
        $params = array();
        ...
        $focus_id = $m_focus ->where("user_id ='%s'",session('user_id'))->getField('customer_id',true);
        //echo $m_focus->getLastSql();
        //dump($focus_id);
        //查询分享给我的
        $m_share =  M('customerShare');
        $sharing_id = session('role_id');
        $m_customer_share = $m_share ->select();
        //echo $m_share->getLastSql();
        //dump($m_customer_share);
        ...
        //dump($share_customer_ids);

        //视图显示筛选
        switch ($by) {
            case 'today' : $where['create_time'] =  array('gt',strtotime(date('Y-m-d', time())));
                $admin['create_time'] =  array('gt',strtotime(date('Y-m-d', time())));break;
            case 'week' : $where['create_time'] =  array('gt',(strtotime(date('Y-m-d')) - (7) * 86400));
                $admin['create_time'] =  array('gt',(strtotime(date('Y-m-d')) - (7) * 86400)); break;
            case 'month' : $where['create_time'] = array('gt',strtotime(date('Y-m-01', time())));
                $admin['create_time'] = array('gt',strtotime(date('Y-m-01', time())));break;
            case 'add' : $order = 'create_time desc'; break;
            case 'update' : $order = 'update_time desc'; break;
            case 'sub' : $where['owner_role_id'] = array('in',implode(',', $below_ids));
                $admin['owner_role_id'] = array('in',implode(',', $below_ids)); break;
            case 'deleted' : $where['is_deleted'] = 1;
                $admin['is_deleted'] = 1;break;
            case 'me' : $where['owner_role_id'] = session('role_id');
                $admin['owner_role_id'] = session('role_id');break;
            case 'focus' : $where['customer_id'] = array('in',$focus_id);
                $admin['customer_id'] = array('in',$focus_id);break;
            case 'share' : $where['customer_id'] = array('in',$customerid);
                $admin['customer_id'] = array('in',$customerid);break;
            case 'myshare' : $where['customer_id'] = array('in',$share_customer_ids);
                $admin['customer_id'] = array('in',$share_customer_ids);break;
            default :
                if($this->_get('content') == 'resource'){
                    $where['_string'] = "customer.owner_role_id=0 or customer.update_time < $outdate";
                    //$all_ids[] = "";
        ...
        //dump($where);
        if($by == 'deleted') unset($where['update_time']);
        if($this->_get('content') == 'resource') unset($where['owner_role_id']);
        if($this->_get('content') != 'resource'){
            if($by != 'deleted'){
            ...
            if ($this->_request('state')){
                ...
                switch ($condition) {
                    case "is" : $where[$field] = array('eq',$search);
                        $admin[$field] = array('eq',$search);break;
                    case "isnot" :  $where[$field] = array('neq',$search);
                        $admin[$field] = array('neq',$search);break;
                    case "contains" :  $where[$field] = array('like','%'.$search.'%');
                        $admin[$field] = array('like','%'.$search.'%');break;
                    case "not_contain" :  $where[$field] = array('notlike','%'.$search.'%');
                        $admin[$field] = array('notlike','%'.$search.'%');break;
                    ...
                    case "is_empty" :  $where[$field] = array('eq','');
                        $admin[$field] = array('eq','');break;
                    case "is_not_empty" :  $where[$field] = array('neq','');
                        $admin[$field] = array('neq','');break;
                    case "gt" :  $where[$field] = array('gt',$search);
                        $admin[$field] = array('gt',$search);break;
                    case "egt" :  $where[$field] = array('egt',$search);
                        $admin[$field] = array('egt',$search);break;
                    case "lt" :  $where[$field] = array('lt',$search);
                        $admin[$field] = array('lt',$search);break;
                    case "elt" :  $where[$field] = array('elt',$search);
                        $admin[$field] = array('elt',$search);break;
                    ...
                    case "between" : $where[$field] = array('between',array($search-1,$search+86400));
                        $admin[$field] = array('between',array($search-1,$search+86400));break;
                    case "nbetween" : $where[$field] = array('not between',array($search,$search+86399));
                        $admin[$field] = array('not between',array($search,$search+86399));break;
                    case "tgt" :  $where[$field] = array('gt',$search+86400);
                        $admin[$field] = array('gt',$search+86400);break;
                    default : $where[$field] = array('eq',$search); $admin[$field] = array('eq',$search);
                }
        ...
        if(trim($_GET['act'] == 'sms')){
            ...
            if(vali_permission('customer', 'export')){
                ...
                $export_limit = ($_GET['export_limit']);
            ...
            if(!isset($_GET['by'])&&session(position_id)==1){
            ...
            $this->field_list = getMainFields('customer');
            $this->alert = parseAlert();
            if(session('position_id') == 21 || session('position_id') == 15 || session('position_id') == 14){
                $this->display('agent');
            }else{
                $this->display();
            }
    ...
    public function listDialog(){
        ...
        if(!empty($business_id)){
            ...
            $customer = $m_customer->where("customer_id = '%s' and is_deleted = 0 and company = '%s'",$customer_id,$_SESSION['company'])->order('create_time desc')->limit(10)->select();
        }else{
            $customer = $m_customer->where("owner_role_id in (%s) and is_deleted = 0 and company ='%s'",implode(',',$underling_ids),$_SESSION['company'])->order('create_time desc')->limit(10)->select();
        ...
        foreach($customer as $k=>$v){
            ...
            if(!empty($v['contacts_id'])){
                ...
                $contacts_customer = $m_r_contacts_customer->where("customer_id = '%s'",$v['customer_id'])->limit(1)->order('id desc')->select();
    ...
    public function view(){
        ...
        if ('0' == $customer_id) {
            ...
            //查询客户数据
            $customer = D('CustomerView')->where("customer.customer_id = '%s'", $customer_id)->find();
            //取得字段列表
            $field_list = M('Fields')->where('model = "customer"')->order('order_id')->select();
            //查询固定信息
            $customer['owner'] = D('RoleView')->where('role.role_id = %d', $customer['owner_role_id'])->find();
            ...
            //合并客户、联系人附件
            $customer_file_ids = M('rCustomerFile')->where("customer_id = '%s'", $customer_id)->getField('file_id', true);
            ...
            $task_ids = M('rCustomerTask')->where("customer_id = '%s'", $customer_id)->getField('task_id', true);
            ...
            $event_ids = M('rCustomerEvent')->where("customer_id = '%s'", $customer_id)->getField('event_id', true);
            ...
            $customer['business'] = M('business')->where("customer_id = '%s' and is_deleted=0", $customer['customer_id'])->select();
            ...
            //合并客户、商机、联系人沟通日志
            $customer_log_ids = M('rCustomerLog')->where("customer_id = '%s'", $customer_id)->getField('log_id', true);
            ...
            $customer['receivables'] = D('ReceivablesView')->where("receivables.customer_id = '%s' and receivables.is_deleted=0", $customer['customer_id'])->select();
            ...
            $customer['payables'] = D('PayablesView')->where("payables.customer_id = '%s' and payables.is_deleted=0", $customer['customer_id'])->select();
            ...
            $customer['cares'] = D('CaresView')->where("customer_cares.customer_id = '%s'", $customer['customer_id'])->select();
            ...
            $contacts_ids = M('rContactsCustomer')->where("customer_id = '%s'", $customer_id)->getField('contacts_id', true);
    ...
    public function excelExport($customerList=false){
        ...
        foreach ($list as $k => $v) {
            ...
            $contactsIdArr = $m_rContactsCustomer->where("customer_id = '%s'", $v['customer_id'])->getField('contacts_id',true);
    ...
    public function getCurrentStatus(){
        $this->ajaxReturn((session('export_status')), 'success', 1);
            ...
            if ($allRow <= 2) {
                ...
                for($currentRow = 3;$currentRow <= $allRow;$currentRow++){
                    ...
                    foreach($field_list as $field){
                        ...
                        // if ($field['is_main'] == 1){
                        // $data[$field['field']] = ($field['form_type'] == 'datetime' && $info != null) ? (PHPExcel_Shared_Date::ExcelToPHP($info))-8*60*60 : $info;
                        // }else{
                        // $data_date[$field['field']] = ($field['form_type'] == 'datetime' && $info != null) ? (PHPExcel_Shared_Date::ExcelToPHP($info))-8*60*60 : $info;
                        // }
                    ...
                    if ($m_customer->create($data) && $m_customer_data->create($data_date)) {
                        ...
                        //添加联系人
                        $m_contacts = M('contacts');
                        $contacts_data['creator_role_id'] = ($_POST['owner_role_id']);
                        ...
                        //设置首要联系人
                        $m_customer->where("customer_id = '%s'", $customer_id)->setField('contacts_id', $contacts_id);

                    }else{
                        if($this->_post('error_handing','',0) == 0){
                ...
                $this->success('导入成功','Customer/index');
            }
        }else{
            $this->display();
        }
    ...
    public function revert(){
        ...
        if ($customer_id) {
            $m_customer = M('customer');
            $customer = $m_customer->where("customer_id = '%s'", $customer_id)->find();
            if ($customer['delete_role_id'] == session('role_id') || session('?admin') ||($customer['company'] == session('company') && session('user_id')==session('parent_id'))) {
                if (isset($customer['is_deleted']) || $customer['is_deleted'] == 1) {
                    if ($m_customer->where("customer_id = '%s'", $customer_id)->setField('is_deleted', 0)) {
    ...
    public function getCustomerList(){
        ...
        //获取下级和自己的客户列表,搜索
        $customerList = M('customer')->where("owner_role_id in (%s) and is_deleted = 0 and company = '%s'", implode(',', $idArray),$_SESSION['company'])->select();
    ...
    public function cares(){
        ...
        foreach ($list as $k => $v) {
            $list[$k]["customer"] = M('customer')->where("customer_id = '%s'", $v['customer_id'])->find();
    ...
    public function caresAdd(){
        ...
        if($this->isPost()){
            ...
            if(!empty($customer['contacts_id'])){
                ...
                $contacts_customer = $m_r_contacts_customer->where("customer_id = '%s'",$customer['customer_id'])->limit(1)->order('id desc')->select();
                $contacts = $m_contacts->where('is_deleted = 0 and contacts_id = %d',$contacts_customer[0]['contacts_id'])->find();
    ...
    public function caresEdit(){
        $care_id = $_POST['care_id'] ? ($_POST['care_id']) : ($_GET['id']);
        ...
        if ($this->isPost()) {
            ...
            if($care_id>0){
                ...
                $care['customer'] = M('customer')->where("customer_id = '%s'", $care['customer_id'])->find();
    ...
    public function analytics(){
        ...
        if($role_id == "all") {
            if(session('position_id')==14 || session('position_id') == 15 ||session('position_id')== 16 || session('position_id') == 21){
                //$roleList = getuser3($type,$typeid);
                //dump($position);
                $roleList = getTJ($position);
            }else{
                $roleList = getTJ();
            }
            $role_id_array = array();
...
//		dump($busi_customer_array);
        ...
        //$where_industry['company'] = $_SESSION['company'];
        ...
        //     $roleList[$roleId] = getUserByRoleId($roleId['role_id']);
        // }
        //dump($roleList);
    ...
    public function customerlock(){
        if(($_GET['customer_id'])){
            $m_customer = M('Customer');
            $customer = $m_customer->where("customer_id = '%s' ", ($_GET['customer_id']))->find();
            if(!empty($customer)){
                if(!in_array(session('role_id'), getSubRoleId(true)) && !session('?admin'))
                    alert('error', L('HAVE NOT PRIVILEGES'), $_SERVER['HTTP_REFERER']);
                if($customer['is_locked']){
                    if($m_customer->where("customer_id = '%s'", ($_GET['customer_id']))->setField('is_locked',0)){
                        alert('success', L('UNLOCKING_SUCCESS'), $_SERVER['HTTP_REFERER']);
                    }else{
                        alert('error', L('UNLOCKING_FAILD'), $_SERVER['HTTP_REFERER']);
                    }
                }else{
                    if($m_customer->where("customer_id = '%s'", ($_GET['customer_id']))->setField('is_locked',1)){
    ...
    public function getCustomerOriginal	(){
        //个人面板
        $dashboard = M('user')->where("user_id = '%s'", session('user_id'))->getField('dashboard');
//		echo M('user')->getLastSql();
//		dump($dashboard);exit;
        ...
        foreach($originalArr as $v){
            ...
            $customerArr['series'][] = array('value'=>($origin_count), 'name'=>$v);
    ...
    public function share(){
        if($this->isPost()){
            ...
            if ($_SESSION['department_id'] == '3'){
                $departments_list = M('roleDepartment')->where('department_id = 3')->select();
            }else{
                $departments_list = M('roleDepartment')->where('department_id != 3')->select();
            }
            ...
            foreach($departments_list as $k=>$v){
                $roleList = $d_role->where("position.department_id = %d and user.company = '%s'", $v['department_id'],$_SESSION['company'])->select();
    ...
    public function close_share(){
        if($this->isPost()){
            ...
            if (empty($customer_ids)) {
                ...
                $is_deleted = $m_share ->where(array('customer_id'=>array('in',$_POST['customer_id'])))->delete();
                if($is_deleted){
            ...
            if (empty($customer_id)) {
                ...
                $is_deleted = $m_share ->where("customer_id = '%s'",$customer_id)->delete();
                if($is_deleted){
                    alert('success','关闭共享成功！',$_SERVER["HTTP_REFERER"]);
                }else{
                    alert('error','关闭共享失败！',$_SERVER["HTTP_REFERER"]);
    ...
    public function batchfocus(){
        if($this->isPost()){
            ...
            $i=0;
            foreach($customer_ids as $v){
                if($m_focus->where('customer_id = "%s" and user_id ="%s"', $v,session('user_id'))->count() <= 0){
                    $data['customer_id'] = $v;
                    $data['user_id'] = session('user_id');
                    $data['focus_time'] = time();
                    $role = M('customer')->where('customer_id = "%s"',$v)->getField('owner_role_id');
                    if(session('company') !== M('user')->where("role_id = '%d'",$role)->getField('company')){
                        alert('error', "部分客户没有权限",$_SERVER['HTTP_REFERER']);
                    }
                    $m_focus ->add($data);
                    $i++;
            ...
            $m_focus = M('customerFocus');
            $customer_id = $_GET['customer_id'];
            $role = M('customer')->where('customer_id = "%s"',$customer_id)->getField('owner_role_id');
            if('' == $customer_id){
                alert('error','参数错误', $_SERVER['HTTP_REFERER']);
            }
            //dump(M('user')->where("role_id = '%d'",$role)->getField('company'));
            if(session('company') !== M('user')->where("role_id = '%d'",$role)->getField('company')){
                alert('error', '没有权限',$_SERVER['HTTP_REFERER']);
            }
            $i=0;
            if($m_focus->where('customer_id = "%s" and user_id ="%s"', $customer_id,session('user_id'))->count() <= 0){
                //echo $m_focus->getLastSql();exit;
                $data['customer_id'] = $customer_id;
                $data['user_id'] = session('user_id');
                $data['focus_time'] = time();
                $m_focus ->add($data);
    ...
    public function batchclose(){
        if($this->isPost()){
            ...
            foreach($customer_ids as $v){
                if($m_focus->where('customer_id = "%s" and user_id ="%s"', $v,session('user_id'))->count() > 0){
                    $m_focus ->where('customer_id = "%s" and user_id ="%s"', $v,session('user_id'))->delete();
                    $i++;
                }
            }
            if($i >0){
            ...
            $i=0;
            if($m_focus->where('customer_id = "%s" and user_id ="%s"', $customer_id,session('user_id'))->count() > 0){
                $m_focus ->where('customer_id = "%s" and user_id ="%s"', $customer_id,session('user_id'))->delete();
                $i = 1;
            }
            if($i >0){
                alert('success',L('CANCEL_THE_ATTENTION'), $_SERVER['HTTP_REFERER']);
    ...
    public function sync(){
        $sync['userId'] = $_SESSION['parent_id'];
        ...
        $html = file_get_contents(C('URL').'/manager//order/getBuyerList', false, $context);
        $html = json_decode($html,true);

        $m_customer = D('Customer');
        $m_customer_data = D('CustomerData');
        $count = count($html['data']);

        //dump($html);exit;
        for($i=0;$i<$count;$i++){
            $data['owner_role_id'] = $_SESSION['role_id'];
            $data['creator_role_id'] = $_SESSION['role_id'];
            $data['company'] = $_SESSION['company'];
            $data['origin'] = '逸管家订单';
            $data['create_time'] = time();
            $data['update_time'] = time();

            $data['name'] = $html['data'][$i]['userNickName'];
            $data['crm_mnpxte'] = $html['data'][$i]['userRealName'];
            $data['address'] = $html['data'][$i]['userAddress'];
            $data['customer_id'] = guid();
            $data['crm_uqqujl'] = $html['data'][$i]['userTelephone'];

            //判断是否重复
            if(!$m_customer->where("name = '%s' and company = '%s'",$data['name'],$_SESSION['company'])->find()){
                //echo $m_customer->getLastSql();exit;
                $sql = "INSERT INTO `crm_customer` (`owner_role_id`,`creator_role_id`,`company`,`origin`,`create_time`,`update_time`,`name`,`crm_mnpxte`,`address`,`customer_id`,`crm_uqqujl`) VALUES (\"$_SESSION[role_id]\",\"$_SESSION[role_id]\",\"$_SESSION[company]\",'逸管家订单',\"$data[create_time]\",\"$data[update_time]\",\"$data[name]\",\"$data[crm_mnpxte]\",\"$data[address]\",\"$data[customer_id]\",\"$data[crm_uqqujl]\")";
                $sqldata = "INSERT INTO `crm_customer_data` (`customer_id`) VALUES (\"$data[customer_id]\")";
                // dump($sql);
                // dump($sqldata);exit;
                $m_customer->query($sql);
                $m_customer_data->query($sqldata);
                actionLog($data['customer_id']);

            }
        }
        $this->ajaxReturn('success',"同步成功！",1);
    }

    // public function sync(){
    // 	$sync['userId'] = $_SESSION['parent_id'];
    // 	//$sync['userId'] = '00006a96c980406bbaee88bc599a7d27';
    // 	$sync = http_build_query($sync);
    // 	$opts = array(
    // 			'http' => array(
    // 						'method' => 'POST',
    ...
    // 	for($i=0;$i<$count;$i++){
    // 		//判断是否重复
    // 		$name = $html['data'][$j]['userNickName'];
    // 		if(!$m_customer->where("name = '%s' and company = '%s'",$name,$_SESSION['company'])->find()){
    // 			$sql = "INSERT INTO `crm_customer` (`owner_role_id`,`creator_role_id`,`company`,`origin`,`create_time`,`update_time`,`name`,`crm_mnpxte`,`address`,`customer_id`,`crm_uqqujl`) VALUES ";
    // 			$sqldata = "INSERT INTO `crm_customer_data` (`customer_id`) VALUES";
    // 			// for($i=0;$i<$count;$i++){
    // 				$data['owner_role_id'] = $_SESSION['role_id'];
    // 				$data['creator_role_id'] = $_SESSION['role_id'];
    // 				$data['company'] = $_SESSION['company'];
    ...
    // 				$data['crm_mnpxte'] = $html['data'][$i]['userRealName'];
    // 				$data['address'] = $html['data'][$i]['userAddress'];
    // 				$data['customer_id'] = $html['data'][$i]['sid'];
    // 				$data['crm_uqqujl'] = $html['data'][$i]['userTelephone'];
    // 				$sql .="(\"$_SESSION[role_id]\",\"$_SESSION[role_id]\",\"$_SESSION[company]\",'逸管家订单',\"$data[create_time]\",\"$data[update_time]\",\"$data[name]\",\"$data[crm_mnpxte]\",\"$data[address]\",\"$data[customer_id]\",\"$data[crm_uqqujl]\"),";
    // 				$sqldata .="(\"$data[customer_id]\"),";
    // 			// };
    // 			$sql = substr($sql,0,strlen($sql)-1);
    // 			$sqldata = substr($sqldata,0,strlen($sqldata)-1);
    // 			// dump($sqldata);
    // 			//dump($sql);exit;
    // 			$haha = $m_customer->query($sql);
    // 			$m_customer_data->query($sqldata);
    // 			if(!$haha){
    // 				$this->ajaxReturn('error',"暂无数据更新!",0);
    // 			}
    // 			//echo $m_customer->getLastSql();exit;
    // 		}
    // 	};
