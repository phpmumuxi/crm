...
class NavigationAction extends Action {
	...
	public function index() {
		if(isset($_GET['postion_top']) || isset($_GET['postion_user']) || isset($_GET['postion_more'])){
			...
			if($user->where("user_id = '%s'", session('user_id'))->setField('navigation',$navigation)){
			...
			$value = $user->where("user_id = '%s'", session('user_id'))->getField('navigation');
			//echo $user->getlastSql();
			$menu = unserialize($value);
			//dump($menu);
			$list = $navigation->select();
			//echo $navigation->getLastSql();
			...
			$simple_menu = M('User')->where("user_id = '%s'", session('user_id'))->getField('simple_menu');
    ...
    //	dump($user);
		...
		$navigation = serialize($menu);  //serialize 返回字符串
		if($user->where("user_id = '%s'", session('user_id'))->setField('simple_menu',$navigation)){
			$this->ajaxReturn('1', L('SAVE_THE_SUCCESS'), 1);
		}else{
			$this->ajaxReturn('0', L('SAVE_THE_FAILURE'), 0);
		}
...
//			echo $navigation->getLastSql();exit;
