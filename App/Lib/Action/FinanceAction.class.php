...
class FinanceAction extends Action{
	public function _initialize(){
		$action = array(
			'permission'=>array(),
			'allow'=>array('changecontent', 'listdialog', 'revert', 'adddialog', 'analytics', 'checkout','getmonthlyreceive','getyearreceivecomparison','getreceivablesmoney','getpayablesmoney','completedelete','delete','agentanalytics','getcompany','getcompanyuser','analytics2')
			);
	...
	public function changecontent(){
		...
		switch ($this->type) {
			...
			$where['receivables.company'] = $_SESSION['company'];
			...
			break;
			case 'payables' :
			$where['payables.status'] = array('neq',2);
			$payables = D('PayablesView');
			$where['payables.company'] = $_SESSION['company'];
			$list = $payables->order($order)->where($where)->page($p.',10')->select();
				//echo $payables->getLastSql();exit;
			foreach($list as $k=>$v){
				$list[$k]['owner'] = getUserByRoleId($v['owner_role_id']);
				$list[$k]['pay_time'] = date("Y-m-d",$v['pay_time']);
			}
	...
	public function index(){
		if(session('position_id') == 14 || session('position_id') == 15){
        if(session('position_id')==14){
       	...
       	if(I('get.company') && I('get.company') !== 'all'){
			...
			if($role_id == "all") {
				...
				$where_previousyear_shoukuan['owner_role_id'] = $where_role_id;
			}else{
				$role_id_array['0'] = $role_id;
				$where_shoukuan['owner_role_id'] = $role_id;
				$where_previousyear_shoukuan['owner_role_id'] = $role_id;
			}
			$year = date('Y');
			$moon = 1;
			...
			$where_thisyear_shoukuan['creator_role_id'] = array('in',getSubRoleId3());
			while ($moon <= 12){
				if($moon == 12) {
				...
				//当前登录人下属
				//$where_thisyear_shoukuan['creator_role_id'] = array('in',getSubRoleId3());
				//dump($where_thisyear_shoukuan);
				$thisyear_shoukuanList = $m_shoukuan->where($where_thisyear_shoukuan)->select();
				//dump($thisyear_shoukuanList);
				//echo $m_shoukuan->getLastSql();
		...
		//原index文件	
			...
			switch ($this->type) {
				...
				$where['receivingorder.company'] = $_SESSION['company'];
				...
				$where['paymentorder.company'] = $_SESSION['company'];
		...
		public function add(){
			switch ($this->type) {
				...
				if($_POST['submit']){
					...
					if(empty($data['price'])){
						alert('error','请填写应收款金额', $_SERVER['HTTP_REFERER']);
					}
					$data['customer_id'] = ($_POST['customer_id'])?($_POST['customer_id']):alert('error',L('PLEASE_SELECT_CUSTOMERS'),$_SERVER['HTTP_REFERER']);
					...
					if($id = $receivables->add($data)){
						if(intval($_POST['check_add_order']) == 1){
							$data_order['name'] = (trim($_POST['order_name']) && (trim($_POST['order_name']) != L('AUTOMATIC_GENERATION')))?trim($_POST['name']):'crm'.date('Ymd').date('His').mt_rand(1000,9999);
							//dump($_POST['order_money']);
							if(!$_POST['order_money']){
								$receivables->where('receivables_id ='.$id)->delete();
								//echo $receivables->getLastSql();exit;
							}

							$data_order['money'] = trim($_POST['order_money'])?trim($_POST['order_money']):alert('error',L('请填写收款单金额'),$_SERVER['HTTP_REFERER']);
							...
							$data_order['company'] = $_SESSION['company'];
				...
				if($_POST['submit']){
					...
					$data['name'] = (trim($_POST['name']) && (trim($_POST['name']) != L('AUTOMATIC_GENERATION')))?trim($_POST['name']):'crm'.date('Ymd').date('His').mt_rand(1000,9999);
		...
		public function listdialog(){
			...
			switch ($this->type) {
				case 'receivables' :
				$list = $receivables->where("receivables.is_deleted = 0 and receivables.status <> 2 and receivables.owner_role_id in($all_ids) and receivables.company ="."'$_SESSION[company]'")->order('receivables.update_time desc')->limit(10)->select();
				//echo $receivables->getLastSql();
				$count = $receivables->where("receivables.is_deleted = 0 and receivables.status <> 2 and receivables.owner_role_id in($all_ids)and receivables.company ="."'$_SESSION[company]'")->count();
				...
				$this->payablesList = $payables->where("payables.is_deleted = 0 and payables.status <> 2 and payables.owner_role_id in($all_ids)and payables.company ="."'$_SESSION[company]'")->order('payables.update_time desc')->limit(10)->select();
				//echo $payables->getLastSql();
				$count = $payables->where("payables.is_deleted = 0 and payables.status <> 2 and payables.owner_role_id in($all_ids) and payables.company ="."'$_SESSION[company]'")->count();
				//echo $payables->getLastSql();
				$this->total = $count%10 > 0 ? ceil($count/10) : $count/10;
				$this->count_num = $count;
	...
	public function analytics(){
		...
		while ($moon <= 12){
			...
			$thisyear_shoukuanList = $m_shoukuan->where($where_thisyear_shoukuan)->where('company = "%s"',session('company'))->select();
			//echo $m_shoukuan->getLastSql();
			$previousyear_shoukuanList = $m_shoukuan->where($where_previousyear_shoukuan)->where('company = "%s"',session('company'))->select();
			$thisyear_fukuanList = $m_fukuan->where($where_thisyear_shoukuan)->where('company = "%s"',session('company'))->select();
			$previousyear_fukuanList = $m_fukuan->where($where_previousyear_shoukuan)->where('company = "%s"',session('company'))->select();
	...
	public function getcompany(){
		if($_GET['userId']){
			$company = M('user')->where("parent_id = '%s' ",$_GET['userId'])->field('user_id,name,role_id,company')->select();
			//echo M('user')->getLastSql();exit;
			$this->ajaxReturn($company,'查询成功', 1);
		}else{
			$this->ajaxReturn($company,'查询失败', 0);
		}	
	}
	...
	public function getcompanyuser(){
		if($_GET['company']){
			$companyUser = M('user')->where('parent_id = "%s"',$_GET['company'])->field('user_id,name,role_id,company')->select();
			//echo M('user')->getLastSql();
			$this->ajaxReturn($companyUser,'查询成功',1);
		}else{
			$this->ajaxReturn($companyUser,'查询失败', 0);
		}
	}
