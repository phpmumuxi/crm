	...
	public function add(){
		...
		if($_POST['submit']){
			if($_POST['r']){
				...
				$m_log->company = $_SESSION['company'];
				if($log_id = $m_log->add()){
					//echo $m_log->getLastSql();exit;
					$m_id = $module . '_id';
					$data['log_id'] = $log_id;
					$data[$m_id] = $model_id;
					if($m_r -> add($data)){
				...
				if($log_id = $m_log->add()){
					...
					if ($data['customer_id']) {
						...
						M('customer')->where("customer_id ='%s'",$_POST['customer_id'])->setField('update_time',time());
					}
		...
		//沟通日志  (日志类型中 type = 1 为沟通日志)
			...
			foreach($list as $k=>$v){
				...
				if(!empty($r_customer_log)){
					$customer = $m_customer->where("customer_id = '%s'",$r_customer_log['customer_id'])->find();
		...
		if($this->isPost()){
			...
			if($log_id = $log->add()){
				...
				if (intval($_POST['customer_id'])) {
					M('RCustomerLog')->add(array('customer_id'=>($_POST['customer_id']),'log_id'=>$log_id));
				}
