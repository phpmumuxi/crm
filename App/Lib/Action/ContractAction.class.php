...
class ContractAction extends Action {
	public function _initialize(){
		$action = array(
			'permission'=>array(),
			'allow'=>array('changecontent','listdialog','getcontractlist','completedelete')
		);
	...
	public function delete(){
		...
		if ('' == $contract_ids) {
			...
			//濡傛灉鍚堝悓涓嬫湁浜у搧锛岃储鍔″拰鏂囦欢淇℃伅锛屾彁绀哄厛鍒犻櫎浜у搧锛岃储鍔″拰鏂囦欢鏁版嵁銆�
			...
			foreach($contract_ids as $k=>$v){
				...
				$contract_product = $m_r_contract_product->where('contract_id = %d',$v)->select();//鍚堝悓鍏宠仈鐨勪骇鍝佽褰�
				$contract_file = $m_r_contract_file->where('contract_id = %d',$v)->select();//鍚堝悓鍏宠仈鐨勬枃浠�
				$contract_receivables = $m_receivables->where('is_deleted <> 1 and contract_id = %d',$v)->select();//鍚堝悓鍏宠仈鐨勫簲鏀舵
				$contract_payables = $m_payables->where('is_deleted <> 1 and contract_id = %d',$v)->select();//鍚堝悓鍏宠仈鐨勫簲浠樻
	...
	public function index(){
		...
		$where['contract.company'] = $_SESSION['company'];
...
//            echo $contract->getLastSql();exit();
	...
	public function getcontractlist(){
        ...
        $where['contract.is_deleted'] = 0;
        $where['contract.company'] = $_SESSION['company'];
		$list = $contract->where($where)->select();
        //echo $contract->getLastSql();
