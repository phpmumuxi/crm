...
//		2.(多个)新增一个关于第三方公司的字段
	...
	public function index(){
        ...
        if($_GET['pagenums']){
        	$this->assign('pagenums',$_GET['pagenums']);
        }
        if($_GET['type']){
        ...
        if($_GET['commodityType']){
        	$this->assign('commodityType',$_GET['commodityType']);
        }else{
        	$this->assign('commodityType','1');
        }
		if(session('position_id') == 19 || session('position_id') == 20 || session('position_id') == 16){
			//1代表自己产品，2代表代理商产品
			$other = $_GET['other']?$_GET['other']:1;
			session('other',$other);
			$this->assign('position',session('position_id'));
			$this->assign('url',C('URl'));
			$this->assign('productuserid',session('user_id'));
	...
	public function editservice(){
		if($_GET['sid']){
			$data['sid']	= $_GET['sid'];
			$data['module']	= 'community_goods_extend';
			$result = httpRequest(C('URL').'/manager/serviceGoods/getGoodsBase',30,$data);
// 			var_dump($result['data']);
			if($result['success']){
				$this->assign('sid',$_GET['sid']);
				$this->assign('model','community_goods_extend');
				$this->assign('serviceinfo',$result['data']);
			...
			//echo session('company');echo $_GET['company'];exit;
...
//			echo G('begin','end').'s';
    ...
    public function manager(){
        if(session('position_id') == 19 || session('position_id') == 20 || session('position_id') == 16 || session('position_id') == 14){
            ...
            $user_list = $d_user->where($where)->where("user.name LIKE '%$username%' OR user.telephone LIKE '%$username%'")->select();
            $userId='';
            ...
            //print_r($data);exit;
            $list=send_post(C('URL').'/manager/ticket/queryTicketList',$data);
            //print_r($list);exit;
            ...
            if(session('company')){
            	...
            	$companyUserListId = array();
            	foreach ($userList as $val){
            		array_push($companyUserListId,$val['user_id']);//同一个店铺的用户id集合
            	}
            }
            $this->assign('companyUserListId',$companyUserListId);
    ...
    public function addmanager(){
        if($this->isPost()){
            ...
            $data['tradeClassHeadArrary']='[{"value":"5500000000","text":"优惠打折"}]';
            $data['ticketTypeName']='优惠打折';
            //print_r($data);exit;
            $status=send_post(C('URL').'/manager/ticket/saveCommunityTicket',$data);
            if ($status) {
                $this->ajaxReturn(1);
            }else {
                $this->ajaxReturn(0);
    ...
    public function updatemanager(){
        $data['sid']=$_GET['sid'];
        $data['module']='community_ticket';
        $list=httpRequest(C('URL').'/manager/serviceGoods/getGoodsBase',30,$data);
        
     
        if($list['success']){
        	$this->assign('sid',$_GET['sid']);
        	$this->assign('model','community_ticket');
        	$this->assign('serviceinfo',$list['data']);
    ...
    public function upper(){
        if($_POST['id']){
            ...
            $status=send_post(C('URL').'/manager/trade/editorSoldOut',$data);
            if ($status) {
    ...
    public function del(){
        if($_POST['id']){
            $data['serviceIds'] = $_POST['id'];
            $status=send_post(C('URL').'/manager/trade/deleteService',$data);
            if ($status) {
                return $this->ajaxReturn();
            }
        }else{
