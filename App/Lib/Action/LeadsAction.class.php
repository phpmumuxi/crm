...
class LeadsAction extends Action{
	...
	public function _initialize(){
		...
		$action = array(
			'permission'=>array(),
			'allow'=>array('analytics','transform','changecontent','getaddchartbyroleid','getownchartbyroleid','check','receive','remove','fenpei','batchreceive', 'assigndialog', 'batchassign', 'revert', 'validate','excelimportdownload','getcurrentstatus','completedelete')
		);
	...
	public function check(){
		...
		if ($this->isAjax()) {
			...
			$leads_commpany_list = $m_leads->where("company = '%s'",$_SESSION['company'])->getField('name', true);

			$customer_commpany_list = $m_customer->where("company='%s'",$_SESSION['company'])->getField('name', true);
		...
		if($this->isPost()){
			...
			if($m_leads->create()){
				if($m_leads_data->create()!==false){
                    ...
                    $m_leads->company = $_SESSION['company'];
	...
	public function delete(){
		...
		if($this->isPost()){
			...
			// dump($leads);exit;
			if (is_array($leads)) {
				if($leads['owner_role_id'] == session('role_id') || session('?admin')|| $leads['creator_role_id'] == session('role_id')){
					$data = array('is_deleted'=>1, 'delete_role_id'=>session('role_id'), 'delete_time'=>time());
	...
	public function index(){
		...
		switch ($by) {
			case 'today' :
				...
				break;
			case 'week' : 
				$where['nextstep_time'] =  array(array('lt',strtotime(date('Y-m-d', time())) + (7) * 86400), array('gt', 0),'and'); 
				break;
			case 'month' : 
		...
		if(I('get.by') == 'public'){
			...
			if (!isset($where['owner_role_id'])) {
				if(!isset($where['_string'])) $where['owner_role_id'] = array('in', implode(',', getSubRoleId(true))); else $where['owner_role_id'] = array('in', '0,'.implode(',', getSubRoleId(true)));
			}
		}
		...
		if(trim($_GET['act'] == 'sms')){
			...
			$list = $d_v_leads->where($where)->page($p.','.$listrows)->order($order)->select();
           	//echo $d_v_leads->getLastSql();
           	// echo (date("Y-m-d H:i:s",1474819200 ));
			...
			if($by == 'deleted') {
				...
				foreach ($list as $k => $v) {
					...
					$list[$k]["customer_name"] = M('customer')->where("customer_id = '%s'", $v['customer_id'])->getField('name');
				}
			}else{
				foreach ($list as $k => $v) {
					$days = 0;
            ...
            $this->role_list = $d_role_view->where('role.role_id in'.'('.implode(',', $below_ids).')'.' AND user.company ='."'$_SESSION[company]'")->select();
	...
	public function listDialog(){
		$m_leads = M('Leads');
		$this->leadsList = $m_leads->where("owner_role_id in (%s) and is_deleted = 0 and is_transformed = 0 and company = '%s'", implode(',', getSubRoleId()),$_SESSION['company'])->order('create_time desc')->limit(10)->select();
		//echo $m_leads->getLastSql();
		$count = $m_leads->where("owner_role_id in (%s) and is_deleted = 0 and is_transformed = 0 and company = '%s'", implode(',', getSubRoleId()),$_SESSION['company'])->order('create_time desc')->count();
