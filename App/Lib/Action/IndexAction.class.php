...
class IndexAction extends Action {
	...
	public function index(){
		...
		$dashboard = $user->where("user_id = '%s'", session('user_id'))->getField('dashboard');
	...
	public function widget_edit(){
		...
		if(isset($_GET['id']) && $_GET['id']!=''){
			...
			if($user->where("user_id = '%s'", session('user_id'))->setField('dashboard', serialize($widgets))){
	...
	public function widget_delete(){
		if(isset($_GET['id']) && $_GET['id']!=''){
			...
			if($user->where("user_id = '%s'", session('user_id'))->setField('dashboard', serialize($widget))){
	...
	public function widget_add(){
		if($this->isPost()){
			if($_POST['widget']){
				...
				if($user->where("user_id = '%s'", session('user_id'))->save($newdashboard)){
