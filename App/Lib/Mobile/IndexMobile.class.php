		...
		if($this->isPost()){
			...
			foreach($log as $k=>$v){
				...
				switch ($v['module_name']) {
					case 'log' :
						$d_log = D('LogView');
						$log_info = $d_log->where('log_id = "%s"',$v['action_id'])->find();
						...
						break;
					case 'customer' :
						$customer_info = $m_customer ->where("customer_id ='%s'",$v['action_id'])->find();
						...
						break;
					case 'contract' :
						$contract_info = $m_contract ->where('contract_id ="%s"',$v['action_id'])->find();
						//客户ID
						...
						break;
					case 'business' :
						$business_info = $m_business ->where('business_id ="%s"',$v['action_id'])->find();
						...
						break;
					case 'leads' :
						$leads_info = $m_leads ->where('leads_id ="%s"',$v['action_id'])->find();
						$tmp['dataa'] = $leads_info['source'];
						$tmp['datab'] = $leads_info['nextstep_time'] ? date("Y-m-d H:i", $leads_info['nextstep_time']):'';
						$tmp['type'] = 7;
						break;
					case 'product' :
						$product_info = $m_product ->where('product_id ="%s"',$v['action_id'])->find();
						...
						break;
					case 'event' :
						$event_info = $m_event ->where('event_id ="%s"',$v['action_id'])->find();
						...
						break;
					case 'user' :
						$user_info = D('UserView') ->where('user.user_id ="%s"',$v['action_id'])->find();
				...
				if($v['module_name'] == 'contract'){
					$aname = M($v['module_name'])->where($v['module_name'].'_id = "%s"',$v['action_id'])->getField('number');
				}else{
					$aname = M($v['module_name'])->where($v['module_name'].'_id = "%s"',$v['action_id'])->getField('name');
				}
