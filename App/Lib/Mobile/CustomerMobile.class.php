...
class CustomerMobile extends Action {
	...
	public function ajax(){
		...
		$contacts_id_list = M('RContactsCustomer')->where("customer_id = '%s'",$customer_id)->select();
		...
		foreach ($list as $k=>$value) {
			...
			$list[$k]['customer'] = M('customer')->where("customer_id = '%s'", $customer_id)->field('name')->find();
		}
		...
		if($this->isAjax()){
			...
			foreach($customer as $k=>$v){
				...
				if(!empty($v['contacts_id'])){
					...
					$contacts_customer = $m_r_contacts_customer->where("customer_id = '%s'",$v['customer_id'])->limit(1)->field('name,contacts_id')->order('id desc')->select();
	...
	public function delete(){
		...
		if($this->isPost()){
			...
			if($m_customer->where("customer_id = '%s'", $customer_id)->setField($data)){
	...
	public function receive(){
		if($this->isPost()){
			...
			$customer_id = isset($_REQUEST['customer_id']) ? (trim($_REQUEST['customer_id'])) : 0;
			//判断是否符合领取条件
	...
	public function index(){
		...
		if($this->isPost()){
			...
			$focus_id = $m_focus ->where("user_id = '%s'",session('role_id'))->getField('customer_id',true);
			//查询分享给我的
	...
	public function dynamic(){
		if($this->isPost()){
			...
			//沟通日志
			$customer_business = M('business')->where("customer_id = '%s' and is_deleted=0", $customer_id)->select();
			...
			//商机统计
			$business_count = M('business')->where("customer_id = '%s' and is_deleted=0", $customer['customer_id'])->count();
