...
class ContactsMobile extends Action{
	...
	public function radioListDialogs(){
		if($this->isAjax()){
			...
			if($_GET['customer_id']){
				...
				$contacts_id = $rcc->where("customer_id = '%s'",$customer_id )->getField('contacts_id', true);
	...
	public function radioListDialog(){
		if($this->isAjax()){
			...
			$where['is_deleted'] = 0;
			if($_GET['customer_id']){
				$contacts_id = $rcc->where("customer_id = '%s'", ($_GET['customer_id']))->getField('contacts_id', true);
				$where['contacts_id'] = array('in', implode(',', $contacts_id));
	...
	public function index(){
		...
		if($this->isPost()){
			...
			if($_GET['customer_id']){
				$contacts_ids = M('rContactsCustomer')->where("customer_id = '%s'", $_GET['customer_id'])->getField('contacts_id', true);
			...
			if($_POST['customer_id']){
				$contacts_id = $rcc->where("customer_id = '%s'", ($_POST['customer_id']))->getField('contacts_id', true);
				$where['contacts_id'] = array('in', implode(',', $contacts_id));
			}
			...
			foreach ($list as $key=>$value) {
				$customer_id = $rcc->where('contacts_id = %d', $value['contacts_id'])->getField('customer_id');
				$customer_list = $m_customer->where("customer_id = '%s'", $customer_id)->field('name,owner_role_id')->find();
	...
	public function view(){
		...
		if($this->isPost()){
			...
			if (empty($contacts_id)) {
				...
				$owner_role_id = $m_customer->where("customer_id = '%s'",$customer_id)->getField('owner_role_id');
