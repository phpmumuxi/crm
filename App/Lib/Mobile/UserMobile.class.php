...
class UserMobile extends Action {
			...
			if ($user['password'] == md5(trim($_POST['password']) . $user['salt'])){
				if (-1 == $user['status']) {
					...
					if (!is_array($role) || empty($role)) {
						...
						M('user')->where("user_id = '%s'",session('user_id'))->setField(array('token'=>$data['token'],'token_time'=>time()));
	...
	*/
	...
	public function index(){
		if($this->isPost()){
			$user_id = M('User')->where('user_id = "%s"',session('user_id'))->getField('user_id');
			$d_role = D('RoleView');
			$role_info = $d_role->where("user.user_id = '%s'", $user_id)->find();
			if(!$role_info){
	...
	public function update(){
		if($this->isPost()){
			...
			$result = M('User')->where('user_id = "%s"',session('user_id'))->save($data);
			if($result){
			...
			if (is_array($user) && !empty($user)) {
				if (md5(md5($verify_code) . $user['salt']) == $user['password']) {
					if ($_POST['password']) {
						...
						if($m_user->where("user_id = '%s'",$user_id)->save(array('password'=>$password, 'lostpw_time'=>0))){
	...
	public function uploadhead(){
       if($this->isPost()){
	       if (isset($_FILES['img']['size']) && $_FILES['img']['size'] > 0) {
				...
				$m_user = M('User');
				$uid = session('user_id');
				$oldImg = $m_user->where("user_id = '%s'",$uid)->getField('img');
				if($oldImg){
					if (file_exists($oldImg)) {
						@unlink($oldImg);
					}
				}
				$r = $m_user->where("user_id = '%s'",$uid)->setField('img',$imgPath);
				if($r){
