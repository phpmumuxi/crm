	...
	public function index(){
		...
		if($this->isPost()){
			...
			if($_GET['customer_id']){
				...
				$business_ids = M('business')->where("customer_id = '%s' and is_deleted=0", $_GET['customer_id'])->getField('business_id',true);
			...
			foreach($list as $k=>$v){
				$customer_name = M('Customer')->where("customer_id = '%s'",$v['customer_id'])->getField('name');
				if($customer_name){
		...
		if($this->isPost()){
			...
			foreach($info as $k=>$v){
				...
				if($k == 'owner_role_id'){
					...
					$customer_name = M('Customer')->where("customer_id = '%s'",$v)->getField('name');
					$contract_list[$i]['id'] = $v;
